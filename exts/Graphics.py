...
def make_palette(colors):
    ...
    for col, pos in zip(colors, positions):
        ...
        textpos = pilutils.align_bbox((0,0,*img.size), font.getsize(text, **font_kwargs), align=3, margin=30)
...
class Graphics(commands.Cog):
    ...
    async def palette(self, ctx):
        colors = (pilutils.mix(pilutils.random_color(), (255,255,255)) for _ in range(6))
